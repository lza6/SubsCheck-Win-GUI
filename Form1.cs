    ...
    public partial class Form1: Form
    {
        ...
        private async Task CheckGitHubVersionAsync()
        {
            try
            {
                ...
                var result = await 获取版本号("https://github.com/repos/cmliu/SubsCheck-Win-GUI/releases/latest");
        ...
        private async void ReadConfig()//读取配置文件
        {
            ...
            checkBox6.CheckedChanged -= checkBox6_CheckedChanged;// 临时移除事件处理器，防止触发事件
            try
            {
                    ...
                    */
                    ...
                    if (subUrls != null && subUrls.Count > 0)
                    {
                        ...
                        for (int i = 0; i < subUrls.Count; i++)
                        {
                            ...
                            {
                                // 处理GitHub raw链接
                                int index = subUrls[i].IndexOf("raw.githubusercontent.com/");
                                if (index > 0) // 如果找到且不在字符串开头（说明是代理格式）
                                {
                                    // 使用 Uri 来安全地处理路径
                                    try
                                    {
                                        var uri = new Uri(subUrls[i]);
                                        // 获取路径和查询字符串
                                        string pathAndQuery = uri.PathAndQuery;
                                        
                                        // 找到 raw.githubusercontent.com/ 之后的路径
                                        int rawIndex = pathAndQuery.IndexOf("raw.githubusercontent.com/");
                                        if (rawIndex >= 0)
                                        {
                                ...
                                else if (subUrls[i].StartsWith(githubRawPrefix)) // 如果是直接的 GitHub raw URL
                                {
                                    filteredUrls.Add(subUrls[i]);
                                }
                                else
                    ...
                    string uselocalproxy = 读取config字符串(config, "use-local-proxy");
                    if (uselocalproxy != null && uselocalproxy == "true")
                    {
            ...
            checkBox6.CheckedChanged += checkBox6_CheckedChanged;// 重新绑定事件处理器
        }
        ...
        private async Task SaveConfig(bool githubProxyCheck = true)//保存配置文件
        {
            try
            {
                ...
                if (githubProxyCheck && !useLocalProxyMode)
                {
                ...
                // 智能代理模式判断 - useLocalProxyMode已在第616行定义
                // 如果未勾选本地代理，但GitHub代理不可用，自动切换到本地代理模式
                if (!useLocalProxyMode && !githubProxyAvailable)
                {
                    Log("⚠️ GitHub代理全部不可用，自动切换到本地代理模式");
                    useLocalProxyMode = true;

                    // 尝试检测本地代理
                    bool localProxyDetected = await DetectLocalProxyForConfig();
                    if (!localProxyDetected)
                    {
                        Log("⚠️ 未检测到本地代理，订阅链接将使用原始GitHub地址（可能无法访问）");
                    }
                }
                else if (useLocalProxyMode)
                {
                    if (disableGitHubProxyGlobal)
                    {
                        Log("✓ 全局禁用GitHub代理，使用本地代理模式");
                    }
                    else
                    {
                        Log("✓ 使用本地代理模式（所有订阅链接保持原始状态）");
                    }
                }
                else
                {
                    Log($"✓ 使用GitHub代理: {comboBox3.Text}");
                }
                ...
                if (!string.IsNullOrEmpty(textBox1.Text))
                {
                    ...
                    for (int i = 0; i < subUrls.Count; i++)
                    {
                        // 只有在本地代理模式未启用且GitHub代理可用时才进行GitHub代理处理
                        if (!useLocalProxyMode && subUrls[i].StartsWith(githubRawPrefix) && !string.IsNullOrEmpty(githubProxyURL))
                        {
                            string githubPath = subUrls[i].Substring(githubRawPrefix.Length);
                ...
                if (comboBox5.Text.Contains("[内置]"))
                {
                    ...
                    if (!File.Exists(downloadFilePath))
                    {
                        ...
                        // 只有在本地代理模式未启用且GitHub代理可用时才进行GitHub代理处理
                        if (!useLocalProxyMode)
                        {
                            string githubPath = downloadUrl.Substring(githubRawPrefix.Length);
                            ...
                            {
                                // 直接访问型
                                config["mihomo-overwrite-url"] = githubProxyURL + githubPath;
                            }
                            else
                            {
                                // 代理加速型
                                config["mihomo-overwrite-url"] = githubProxyURL + "raw.githubusercontent.com/" + githubPath;
                            }
                        }
                        else
                        {
                            // 使用本地代理时，直接使用原生GitHub链接
                            config["mihomo-overwrite-url"] = downloadUrl;
                        }
                    }
                    else
                    {
                ...
                else if (comboBox5.Text.StartsWith(githubRawPrefix))
                {
                    string githubPath = comboBox5.Text.Substring(githubRawPrefix.Length);
                    ...
                    {
                        // 使用本地代理时，直接使用原生GitHub链接
                        config["mihomo-overwrite-url"] = comboBox5.Text;
                    }
                }
                ...
                config["use-local-proxy"] = checkBox6.Checked;//是否使用本地代理
        ...
        private async Task DownloadSubsCheckEXE()
        {
            ...
            try
            {
                ...
                var result = await 获取版本号("https://github.com/repos/beck-8/subs-check/releases/latest", true);
                if (result.Item1 != "未知版本") 
                {
                    ...
                    if (localProxyPort > 0)
                    {
                        var proxy = new System.Net.WebProxy($"http://127.0.0.1:{localProxyPort}");
                        handler.Proxy = proxy;
                        Log($"配置使用本地代理下载: 127.0.0.1:{localProxyPort}");
                    }
                    else
                    {
                    ...
                    using (HttpClient client = new HttpClient(handler)) 
                    {
                        try
                        {
                            ...
                            // 如果勾选了本地代理，先检测本地代理
                            if (checkBox6.Checked && localProxyPort == 0)
                            {
                                Log("开始检测本地代理...");
                                bool localProxyDetected = await DetectLocalProxyForConfig();
                                if (localProxyDetected)
                                {
                                    Log($"✓ 检测到本地代理: 127.0.0.1:{localProxyPort}");
                                }
                                else
                                {
                                    Log("⚠️ 未检测到本地代理，下载可能会失败");
                                }
                            }

                            // 直接构造下载链接
                            string downloadUrl = $"https://github.com/beck-8/subs-check/releases/download/{latestVersion}/subs-check_Windows_i386.zip";

                            if (downloadUrl != null)
                            {
                                // 判断是否使用本地代理模式
                                bool useLocalProxyMode = checkBox6.Checked;

                                // 如果未勾选本地代理，检查GitHub代理是否可用
                                if (!useLocalProxyMode && !string.IsNullOrEmpty(githubProxyURL))
                                {
                                    bool githubProxyAvailable = await TestProxyAvailabilityAsync(comboBox3.Text == "自动选择" ? githubProxyURL.Replace("https://", "").Replace("/", "") : comboBox3.Text);
                                    if (!githubProxyAvailable)
                                    {
                                        Log("⚠️ GitHub代理不可用，将使用原生GitHub链接（可能需要本地代理）");
                                        useLocalProxyMode = true;
                                ...
                                if (!useLocalProxyMode && !string.IsNullOrEmpty(githubProxyURL))
                                {
                                    ...
                                    if (proxyType == 1)
                                    {
                                        // 直接访问型：https://bgithub.xyz/beck-8/subs-check/releases/download/v1.4.6/subs-check_Windows_i386.zip
                                        代理下载链接 = githubProxyURL + "beck-8/subs-check/releases/download/" + latestVersion + "/subs-check_Windows_i386.zip";
                                    }
                                    else
                                    {
                                        // 代理加速型：https://ghp.ci/github.com/beck-8/subs-check/releases/download/v1.4.6/subs-check_Windows_i386.zip
                                        代理下载链接 = githubProxyURL + "github.com/beck-8/subs-check/releases/download/" + latestVersion + "/subs-check_Windows_i386.zip";
                                    }
                                ...
                                {
                                    Log("使用本地代理模式，下载链接将保持原始GitHub地址");
                                }

                                string executablePath = Path.GetDirectoryName(Application.ExecutablePath);

                                // 创建下载请求 - 智能选择下载逻辑
                                Log($"原始下载URL: {downloadUrl}");
                                if (!string.IsNullOrEmpty(代理下载链接))
                                {
                                    Log($"代理下载URL: {代理下载链接}");
                                }
                                Log("开始下载文件...");
                                ...
                                if (useLocalProxyMode || string.IsNullOrEmpty(代理下载链接))
                                {
                                    // 使用本地代理模式或没有可用的GitHub代理，直接使用原生链接
                                    Log($"[尝试1/2] 使用原生GitHub链接（本地代理模式）：{原生下载链接}");
                                    try
                                    {
                                ...
                                {
                                    // 使用GitHub代理模式，尝试多种下载方式
                                    // 第一次尝试：使用代理下载链接 + 当前HttpClient(不使用系统代理)
                                    try
                                    {
                                        Log($"[尝试1/4] 使用GitHub代理下载链接：{代理下载链接}");
        ...
        private async Task<(string LatestVersion, JArray assets)> 获取版本号(string 版本号URL, bool 是否输出log = false)
        {
            ...
            if (localProxyPort == 0)
            {
                if (是否输出log) Log("开始检测本地代理...");
                localProxyPort = await DetectLocalProxyAsync();
                if (localProxyPort > 0)
                {
                    if (是否输出log) Log($"✓ 检测到本地代理: 127.0.0.1:{localProxyPort}");
                }
                else
                {
                    if (是否输出log) Log("✗ 未检测到本地代理");
                }
            }
            ...
            if (localProxyPort > 0)
            {
                var proxy = new System.Net.WebProxy($"http://127.0.0.1:{localProxyPort}");
                handler.Proxy = proxy;
                if (是否输出log) Log($"配置使用本地代理: 127.0.0.1:{localProxyPort}");
            }
            else
            {
                handler.UseProxy = false;
                handler.Proxy = null;
                if (是否输出log) Log("未配置代理，将直接访问");
            }
            ...
            using (HttpClient client = new HttpClient(handler))
            {
                ...
                try
                {
                    ...
                    if (是否输出log)
                    {
                        ...
                        Log($"请求耗时: {stopwatch.ElapsedMilliseconds}ms");
                        Log("响应头信息:");
                    ...
                    if (response.StatusCode == System.Net.HttpStatusCode.Found || response.StatusCode == System.Net.HttpStatusCode.MovedPermanently)
                    {
                        ...
                        if (是否输出log) Log($"检测到重定向: {redirectUrl}");
                        ...
                        if (!string.IsNullOrEmpty(redirectUrl))
                        {
                            ...
                            if (lastSlashIndex > 0)
                            {
                                latestVersion = redirectUrl.Substring(lastSlashIndex + 1);
                                if (是否输出log) Log($"从重定向URL提取到版本号: {latestVersion}");
                            }
                        }
                    }
                    else if (response.IsSuccessStatusCode)
                    {
                        ...
                        if (是否输出log)
                        {
                            Log($"直接访问成功，HTTP {(int)response.StatusCode}");
                            Log($"响应内容长度: {content.Length} 字符");
                            Log($"响应内容预览 (前500字符):");
                            Log(content.Length > 500 ? content.Substring(0, 500) + "..." : content);
                        }
                        ...
                        if (match.Success)
                        {
                            ...
                            if (是否输出log) Log($"从HTML提取到版本号: {latestVersion}");
                        }
                        else
                        {
                            if (是否输出log) Log("未能从HTML中提取版本号", true);
                        }
                    }
                    else
                    {
                ...
                catch (TaskCanceledException ex)
                {
                    if (是否输出log)
                    {
                        Log($"请求超时: 请求在30秒内未完成", true);
                        Log($"异常详情: {ex.Message}", true);
                    }
                }
                // 捕获其他所有异常
                catch (Exception ex)
                {
                    if (是否输出log)
                    {
                        Log($"获取版本信息时出现未预期的错误: {ex.Message}", true);
                        Log($"异常类型: {ex.GetType().Name}", true);
                        Log($"异常详情: {ex}", true);
                    }
                }
        ...
        private void Log(string message, bool isError = false)
        {
            ...
            foreach (var line in lines)
            {
                ...
                {
                    output.AppendLine($"{timestamp} {logType} {line}");
                }
            }
            ...
            if (availableProxies.Count == 0)

                        {
                            ...
                            int[] commonPorts = { 
                                ...
                                10808, 10809, 1080,  // V2Ray常见端口

                                7890, 7891, 7892,  // Clash常见端口

                                1087, 1086, 1085, 1081, 1082, 1083, 1084,  // 其他常见端口

                                7893, 7894, 7895,  // Clash备用端口

                                20808, 20809,  // V2Ray备用端口

                                7897, 7898, 7899,  // 更多Clash端口

                                10810, 10811, 10812  // 更多V2Ray端口
                            ...
                            Log($"尝试检测 {commonPorts.Length} 个常见本地代理端口...");
...

                            {
                                ...
                                Log($"检测到本地代理: 127.0.0.1:{detectedPort}", true);

                                Log($"本地IP: {localIP}", true);

                                Log($"出口IP: {externalIP}", true);

                                

                                // 更新本地代理端口变量

                                localProxyPort = detectedPort;
...

                            {
                                ...
                                Log("未检测到本地代理，建议启动V2Ray/Clash等代理软件", true);
        ...
        // 获取代理的完整URL格式
        private string GetProxyURL(string proxyItem)
        {
            int proxyType = GetProxyType(proxyItem);

            if (proxyType == 1)
            {
                // 直接访问型：https://bgithub.xyz/
                return $"https://{proxyItem}/";
            }
            else
            {
                // 代理加速型：https://ghp.ci/
        ...
        private async Task<ProxyTestResult> TestProxyLatency(string proxyItem)
        {
            ...
            {
                ProxyName = proxyItem,
                ProxyURL = GetProxyURL(proxyItem),
                IsAvailable = false
            };
        ...
        private void button4_Click(object sender, EventArgs e)
        {
            ...
            // 根据主窗口的"使用本地代理"状态设置CheckUpdates的"禁用GitHub代理"状态
            checkUpdatesForm.DisableGitHubProxy = checkBox6.Checked;

            // 设置日志回调，将 CheckUpdates 的日志输出到主窗口
            checkUpdatesForm.LogCallback = (message, isError) => {
                Log(message, isError);
            };
            ...
            checkUpdatesForm.FormClosed += (s, args) => {
                ...
                // 更新全局的禁用GitHub代理设置
                disableGitHubProxyGlobal = checkUpdatesForm.DisableGitHubProxy;
                
                // 如果全局禁用了GitHub代理，自动勾选"使用本地代理"
                if (disableGitHubProxyGlobal)
                {
                    checkBox6.Checked = true;
                }
        ...
        private async Task ProcessComboBox5Selection(bool 汇报Log = false)
        {
            ...
            if (!File.Exists(downloadFilePath))
            {
                ...
                // 判断是否使用本地代理模式
                bool useLocalProxyMode = checkBox6.Checked;
                bool githubProxyAvailable = false;
                string availableProxyURL = "";

                // 如果未使用本地代理模式，检测GitHub代理可用性
                if (!useLocalProxyMode)
                {
                    if (汇报Log) Log("开始检测GitHub代理可用性...");
                    ...
                    // 并发检测所有代理，选择延迟最低的
                    availableProxyURL = await DetectFastestGitHubProxyAsync(proxyItems);
                    githubProxyAvailable = !string.IsNullOrEmpty(availableProxyURL);

                    if (githubProxyAvailable)
                    {
                        if (汇报Log) Log($"检测到可用的GitHub代理: {availableProxyURL}");
                    }
                    else
                    {
                        if (汇报Log) Log("未检测到可用的GitHub代理，将尝试本地代理");
                        useLocalProxyMode = true;
                    }
                }
                else
                {
                    if (汇报Log) Log("使用本地代理模式");
                }

                // 如果切换到本地代理模式，检测本地代理
                if (useLocalProxyMode)
                {
                    if (汇报Log) Log("开始检测本地代理...");

                    bool localProxyDetected = await DetectLocalProxyForConfig();
                    if (localProxyDetected)
                    {
                        if (汇报Log) Log("检测到本地代理，将使用原生GitHub链接");
                    }
                    else
                    {
                        if (汇报Log) Log("未检测到本地代理，仍将尝试原生GitHub链接（可能无法访问）");
                    }
                }
                ...
                if (!useLocalProxyMode && githubProxyAvailable)
                {
                    // 如果未使用本地代理模式且GitHub代理可用，使用GitHub代理
                    string proxyUrl = availableProxyURL + downloadUrl;
                    downloadUrls.Add(proxyUrl);
                    if (汇报Log) Log($"添加GitHub代理下载链接");
                }

                // 添加原生GitHub链接（总是添加，作为回退选项）
                downloadUrls.Add(downloadUrl);
                if (汇报Log) Log($"添加原生GitHub链接");

                bool downloadSuccess = false;

                foreach (string fullDownloadUrl in downloadUrls)
                {
                    bool isProxyUrl = fullDownloadUrl.Contains(availableProxyURL) && !useLocalProxyMode;
                    string urlType = isProxyUrl ? "GitHub代理" : "原生GitHub链接";

                    if (汇报Log) Log($"尝试使用{urlType}下载: {fullDownloadUrl}");
                    ...
                    try
                    {
                        ...
                        // 如果使用本地代理模式且检测到本地代理，配置使用本地代理
                        if (useLocalProxyMode && localProxyPort > 0)
                        {
                            var proxy = new System.Net.WebProxy($"http://127.0.0.1:{localProxyPort}");
                            handler.Proxy = proxy;
                            if (汇报Log) Log($"配置使用本地代理: 127.0.0.1:{localProxyPort}");
                        }
                        else
                        {
                            handler.UseProxy = false;
                        ...
                        using (HttpClient client = new HttpClient(handler))
                        {
                            ...
                            {
                                if (汇报Log) Log($"{urlType}无法访问，尝试下一个...");
                                continue;
                            }
                            ...
                            using (var response = await client.GetAsync(fullDownloadUrl, HttpCompletionOption.ResponseHeadersRead))
                            {
                                if (response.IsSuccessStatusCode)
                                {
                                    ...
                                    Log($"{displayName} 覆写配置文件 下载成功 (使用{urlType})");
                                    downloadSuccess = true;
                                    break; // 下载成功，退出循环
                                }
                                else
                                {
                                    if (汇报Log) Log($"{urlType}下载失败: HTTP {(int)response.StatusCode} {response.ReasonPhrase}");
                                }
                            }
                        }
                    }
                    ...
                    {
                        if (汇报Log) Log($"{urlType}下载失败: {ex.Message}");
                    }
                }

                if (!downloadSuccess)
                {
                    Log($"{displayName} 覆写配置文件 所有下载尝试均失败", true);
                    // 出错时重置进度条
                    progressBar1.Value = 0;
                }
            }
        ...
        /// 检测本地代理状态
        /// </summary>
        private async Task<int> DetectLocalProxyAsync()
        {
            try
            {
                textBox12.Text = "正在检测本地代理...";
                Log("开始检测本地代理...");

                // 常见的本地代理端口列表（V2Ray、Clash等）- 添加更多端口
                int[] commonPorts = { 
                    10808, 10809, 1080,  // V2Ray常见端口
                    7890, 7891, 7892,  // Clash常见端口
                    1087, 1086, 1085, 1081, 1082, 1083, 1084,  // 其他常见端口
                ...
                if (detectedProxy != null)
                {
                    ...
                    textBox12.Text = $"✓ 已检测 | 端口:{detectedPort} | 出口IP:{externalIP} | 本地IP:{localIP}";
                    Log($"✓ 本地代理检测成功: {detectedProxy}");
                    Log($"  本地IP: {localIP}");
                    Log($"  出口IP: {externalIP}");
                    return detectedPort;
                }
                ...
                {
                    textBox12.Text = "✗ 未检测到本地代理";
                    Log("✗ 未检测到本地代理，请确保V2Ray/Clash等代理软件已启动");
                    checkBox6.Checked = false; // 自动取消勾选
                    return 0;
                }
            }
            catch (Exception ex)
        ...
        /// 测试指定的GitHub代理是否可用
        /// </summary>
        private async Task<bool> TestProxyAvailabilityAsync(string proxyItem)
        {
            try
            {
                var stopwatch = System.Diagnostics.Stopwatch.StartNew();
                ...
                using (HttpClient client = new HttpClient())
                {
                    ...
                    if (response.IsSuccessStatusCode)
                    {
                        Log($"✓ GitHub代理可用: {proxyItem} ({stopwatch.ElapsedMilliseconds}ms)");
                        return true;
                    }
                }
            }
        ...
        /// <summary>
        /// 检测本地代理（用于配置保存时，不显示UI）
        /// </summary>
        private async Task<bool> DetectLocalProxyForConfig()
        {
            try
            {
                // 常见的本地代理端口列表（V2Ray、Clash等）- 添加更多端口以提高检测成功率
                int[] commonPorts = { 
                    10808, 10809, 1080,  // V2Ray常见端口
                    7890, 7891, 7892,  // Clash常见端口
                    1087, 1086, 1085, 1081, 1082, 1083, 1084,  // 其他常见端口
                ...
                Log($"开始检测 {commonPorts.Length} 个常见本地代理端口...");

                foreach (int port in commonPorts)
                {
                    if (await TestLocalProxyPort(port))
                    {
                        Log($"✓ 检测到本地代理: 127.0.0.1:{port}");
                        localProxyPort = port; // 更新本地代理端口
                        return true;
                    }
                }
